...
const localStorageKey = 'CUPSHE_REQUEST_HEADER'
...
export const createAxios = ({
  ...
  axiosRetry(axios, {
    ...
    retryCondition: (error) => {
      // true为打开自动发送请求，false为关闭自动发送请求。只有超时和http状态码错误时重试
  ...
  const combine = (params) => {
    return Object.assign({}, baseContext, params)
  }
  ...
  const getRequest = (request) => {
    return (...args) => {
      ...
      return fetch.finally(() => {
        requestQueue.delete(requestKey) // 接口返回后在队列中删除
      })
    }
  }
